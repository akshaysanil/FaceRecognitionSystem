...
def callback(frame: np.ndarray, _: int) -> np.ndarray:
    ...
    for detection,label in zip(detections.xyxy,labels):
        ...
        # facenet_result = f"Face{i}: {result} "
        name = facenet_result
        # if name not in saved_names:
        #     face = frame[y1:y2, x1:x2]
        #     # face = frame[y1 + 20:y2+20 , x1+20:x2+20]
    ...
    # Join all face recognition results into a single string
    # facenet_results_str = " ".join(facenet_results) if facenet_results else "No faces detected"
     # Display the frame
    # cv.imshow("Live Detection", annotated_frame)
    # cv.waitKey(1)  # Wait for 1 ms to show the frame


    return label_annotator.annotate(
        annotated_frame, detections=detections, labels=labels , text_from_facenet = facenet_results)
        # annotated_frame, detections=detections, labels=labels , text_from_facenet = facenet_results_str)
        # annotated_frame, detections=detections, labels=labels)

sv.process_video(
    source_path="testing_video.mp4",
