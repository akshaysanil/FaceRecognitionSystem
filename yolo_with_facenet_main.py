...
def callback(frame: np.ndarray, _: int) -> np.ndarray:
    ...
    for detection in detections.xyxy:
        ...
        print('name >>>>>>>>>>>>>>>>>>> :  ',name)
        print('result_probabilty >>>>>>>>>>>>>>>>>>> : ',result_probabilty)
...
# saving the attandance to the database(locally) when the person cross the line
    ...
    return label_annotator.annotate(
        annotated_frame, detections=detections, labels=labels , text_from_facenet = facenet_results,result_probability = result_probabiltys)
...
# ['Abhishek P', 'Ashlesha P D', 'Deepak A', 'Harsha K S', 'Harshitha H', 'Kathirvel S', 'Namratha S Gowda', 'Pooja Aiyappa', 'Satish S']
